...
public class ContactListDaoDbImpl implements ContactListDao {
    ...
    //transaction = required bc if the second call to the database is not completed
    //in the same transaction, the wrong value can be returned
    //read only = false bc we are making changes 
    public Contact addContact(Contact contact) {
        jdbcTemplate.update(SQL_INSERT_CONTACT,
                ...
                contact.getPhone(),
                contact.getEmail());
        //order must match prepared statement 
        //query the database for the id that was just assigned to the new row in the database
    ...
    public List<Contact> getAllContacts() {
        //use .query when more than one row is expected to return
    ...
    public Contact getContactById(long contactId) {
        try {
            ...
            //if there are not results for the given contact id, return null
            return null;
        }
    }
